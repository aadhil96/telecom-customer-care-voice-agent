...
import ssl
import certifi
from telecom_functions import FUNCTION_MAP
...
def sts_connect():
    ...
    sts_ws = websockets.connect(
        ...
        subprotocols=["token", api_key], ssl=ssl_context
    )
    return sts_ws


def load_config():
    with open("telecom_config.json", "r") as f:
        return json.load(f)
...
async def handle_function_call_request(decoded, sts_ws):
    for function_call in decoded["functions"]:
        ...
        await sts_ws.send(json.dumps(
            create_function_call_response(func_id, func_name, result)
        ))
